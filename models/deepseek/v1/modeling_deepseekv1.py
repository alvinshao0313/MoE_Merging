        ...
        if self.training and self.alpha > 0.0:
            ...
            if self.seq_aux:
                ...
                ce.scatter_add_(1, topk_idx_for_aux_loss, torch.ones(bsz, seq_len * aux_topk,
                                device=hidden_states.device)).div_(seq_len * aux_topk / self.n_routed_experts)
...
class DeepseekMoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList([DeepseekMLP(config, intermediate_size=config.moe_intermediate_size)
